...
function generateTest() {
    if (areParametersValid()) {
        ...
        fs.writeFile(
            ...
            () => {
                console.log(
                    chalk.green("Test launch config is updated locally")
                );
            }
        );
        fs.writeFile(
            ...
            () => {
                console.log(
                    chalk.green(
                        "Test launch config is updated is .vscode folder"
                    )
                );
            }
        );
...
function areParametersValid(): boolean {
    ...
    if (
        commandPrams.every(
            param => !/--fileName=([a-zA-Z0-9(\s)])+/.test(param)
        )
    ) {
        validParams = false;
        console.log(
