...
export class JSDebugMessage extends DebugMessage {
    ...
    line(
        ...
        if (
            ...
            if (currentLineText.split("=")[0].includes(selectedVar)) {
                ...
                return (
                    this.functionOpenedBraceLine(document, selectionLine) + 1
                );
            }
        } else if (
            ...
            return multilineParenthesisVariableLine !== null &&
                this.lineText(
                    document,
                    multilineParenthesisVariableLine - 1
                ).includes("{")
                ? multilineParenthesisVariableLine
                : selectionLine + 1;
        } else if (
            this.lineCodeProcessing.isFunctionDeclaration(currentLineText)
        ) {
            if (
    ...
    private arrayLine(document: TextDocument, selectionLine: number): number {
        ...
        if (nbrOfOpenedBrackets !== nbrOfClosedBrackets) {
            while (currentLineNum < document.lineCount) {
                const currentLineText: string =
                    document.lineAt(currentLineNum).text;
                nbrOfOpenedBrackets += (currentLineText.match(/\[/g) || [])
                    .length;
                nbrOfClosedBrackets += (currentLineText.match(/\]/g) || [])
                    .length;
                currentLineNum++;
                if (nbrOfOpenedBrackets === nbrOfClosedBrackets) {
                    break;
    ...
    private getMultiLineVariableLine(
        ...
        while (currentLineNum >= 0) {
            const currentLineText: string =
                document.lineAt(currentLineNum).text;
            const currentLineParenthesis =
                this.locOpenedClosedElementOccurrences(
                    currentLineText,
                    blockType
                );
            nbrOfOpenedBlockType +=
                currentLineParenthesis.openedElementOccurrences;
            nbrOfClosedBlockType +=
                currentLineParenthesis.closedElementOccurrences;
        ...
        if (
            ...
            if (nextLineTextChars.filter(char => char !== " ").length !== 0) {
                if (
                    ...
                    if (
                        nextLineTextChars[
                            nextLine.firstNonWhitespaceCharacterIndex - 1
                        ] === "\t"
                    ) {
                        return " ".repeat(
                ...
                case "function":
                    if (
                        ...
                        if (
                            ...
                            if (
                                this.lineCodeProcessing.getFunctionName(
                                    currentLineText
                                ).length !== 0
                            ) {
    ...
    detectAll(
        ...
        for (let i = 0; i < documentNbrOfLines; i++) {
            const gofmtMessage: RegExp = /fmt\.Println\(/;
            if (gofmtMessage.test(document.lineAt(i).text)) {
                ...
                for (let j = i; j <= closedParenthesisLine; j++) {
                    ...
                    logMessage.lines.push(
                        document.lineAt(j).rangeIncludingLineBreak
                    );
                }
                if (
                    new RegExp(
